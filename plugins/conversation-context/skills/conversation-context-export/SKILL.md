...
##### 既存コメントのExported byの引き継ぎ

`.dev/contexts/` はgitignoreされており別マシンには存在しないため、別セッションの貢献者はPRコメント本文にしか残っていないことがある。4-2で既存コメントが見つかった場合は、上書きで過去のExported byを失わないよう、下記の投稿・更新の前に既存コメント本文のExported byを取り込む:

- 既存コメント本文を `gh api repos/{owner}/{repo}/issues/comments/{コメントID} -q .body` で取得する
- 「既存ファイルの更新ルール > 共通ルール」の累積ルールに従い、既存コメントのExported byエントリとローカルファイルのExported byを統合する。重複は除去し順序は保持する
- 統合結果でローカルファイル `.dev/contexts/{サニタイズ済みブランチ名}.md` のExported by行を更新してから、下記の投稿・更新を行う

4-2で既存コメントが見つからなかった場合は、取り込むものがないためこの手順をスキップする。

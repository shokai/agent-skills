...
targets.mdとchecklist.mdの `Generated by` を確認し、Codexがセカンドオピニオンとして記録されていなければ、他の作業より先に手順2の「生成したリストのCodexレビュー」を行う。「Codexレビュー未実施」の記録がある場合だけでなく、Codexの実行環境が書かれていない場合や、主エージェント自身がCodexである場合も該当する。

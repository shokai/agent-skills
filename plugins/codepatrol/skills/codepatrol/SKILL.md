...
1. 自分のmodelがOpusまたはFableである。Sonnet・Haiku等の軽量modelは脆弱性の検知能力が足りない
2. Codex CLIがインストールされている。Bashツールで `command -v codex` を実行して確認する
3. CodexのmodelがSolまたはAstra（`gpt-*-sol` / `gpt-*-astra`）で、reasoning effortがmedium以上である。短いプロンプトを作業ツリー外のファイルに書き、`codex exec --ephemeral` にstdinで渡して1回実行し、出力のヘッダから読む。Luna・Terra等の軽量modelや古いmodelは相談相手として足りない
...
- 主エージェントがOpusまたはFableだと確認できない（Sonnet・Haiku、Codex、記録なし等）: 手順2の更新フローで両方を再走査して作り直し、Codexレビューを経る
- 主エージェントは条件を満たすが、Codexの実行環境が上の条件3を満たすと確認できない（「Codexレビュー未実施」の記録がある、Codexの実行環境が書かれていない、modelやreasoning effortの記載がない、条件外のmodel等）: 手順2の「生成したリストのCodexレビュー」を行う
...
targets.mdまたはchecklist.mdを生成・更新したら、両方をファイルに書き出した上で、2つまとめてCodexにレビューさせる。
一方だけを更新した場合も、両方を渡す。
このレビューは省略も代替もできない。Codexのレビューを経ていないリストで手順3以降に進まない。
...
手順4の調査結果をCodexに批判させ、漏れ抜けを検出する。
批判的思考の連鎖により、単独調査では見落とす脆弱性を発見できる。
この手順は省略も代替もできない。Codexのレビューを経ていない調査結果はレポートにしない。
...
- **codex-consultation**: Codex CLIと相談するスキル。手順2のリストレビューと手順5の批判的レビューで使用する。必須で、他のスキルで代替しない

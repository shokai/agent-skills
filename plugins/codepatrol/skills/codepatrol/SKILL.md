...
Codexが回答を返さずに停止した場合の扱いは手順5の「Codexが停止した場合」に従う。レビューできなかった時は生成したファイルをそのまま残し、`Generated by` に「Codexレビュー未実施」と記録してユーザーに報告する。次にCodexが使える時にレビューを行う。他のAgentや自分自身のレビューで代替しない。

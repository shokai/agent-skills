...
領域リストと観点リストは対であり、片方だけでは領域と観点の対応の漏れを判断できない。どちらか一方だけを更新した場合も、両方を渡してレビューさせる。
リストの漏れや粒度の偏りは、以降の全ての調査がその形に沿って進むため、後の手順では気付けない。

Skill toolで `codex-consultation` を呼び出す。Argsには以下を含める:

```
よく相談して。このリポジトリのセキュリティ調査に使う調査対象リストと観点リストを作成した。
以下の2つのファイルを読んで、批判的にレビューしてほしい。
...
2つのリストは対であり、targets.mdの各領域にchecklist.mdの観点を当てて調査する。両者の対応の漏れも指摘してほしい。
```

codex-consultationが利用できない場合は、Skill toolで `subagent-consultation` にフォールバックする。
両方利用できない場合はレビューを省略する。

妥当な指摘をファイルに反映し、`Generated by` に自分のAgent名とmodel名を記入する。レビューを行った場合はconsultation skillが取得した外部Agentの実行環境を併記する。

...
**checklistの領分（生成時・更新時ともに守る）:**

checklistには「仕様・機構の説明（具体名付き）」と「中立な観点（何を確認するか）」だけを書く。特定のbug・脆弱性の断定、悪用可否の仮説、深刻度、既存レポートの発見への言及は**書かない**。それらは手順6のレポートの領分である。

- 機構が弱く見えても「Xという機構がある / ない」という事実記述に留める（例:「`sanitizeFilter` 未使用」「private-IPブロック無し」はOK）
- 「危険」「要重点確認」「〜に悪用できないか」等の表現で特定箇所への過剰注目を促さない。観点は「〜になっていないか」「〜を確認する」という中立な問いで書く
- 理由: checklistがbugを先取りすると、調査者（および手順5の外部Agent）がその箇所に引っ張られ、本当に見るべき別のbugを見落とす。checklistは足場であって発見リストではない
...
3. チェックリストの各観点を当てる:
   ...
   - 作業用checklist.mdの記述（機構の要約・具体名）と実装の齟齬を見つけたら、その場でchecklist.mdを修正する。ただし修正は機構・仕様の事実レベルに留め、発見したbug自体はchecklistに書かずレポートへ書く（checklistの領分は手順2を参照）
...
- 指摘内容を自分でコードを読んで検証する
- 妥当な指摘は手順4の調査結果（＝レポートに書く発見）に反映する。外部Agentの指摘はbug寄りなので、checklistには取り込まない（取り込むとchecklistが発見リスト化し、次回以降の調査を特定箇所へ誘導してしまう。checklistの領分は手順2を参照）

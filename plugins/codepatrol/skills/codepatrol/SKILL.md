...
まず実行環境を確認する。いずれかを満たさなければ、リストの生成・調査・レポート出力のいずれも行わず、満たさない条件をユーザーに報告して停止する:
...
targets.mdとchecklist.mdの `Generated by` を確認し、次のいずれかに該当すれば、他の作業より先に手順2で対処する:

- 主エージェントが上の条件1を満たすと確認できない（Sonnet・Haiku、Codex、記録なし等）: 手順2の更新フローで両方を再走査して作り直し、codex-consultationでのレビューを経る
- 主エージェントは条件を満たすが、Codexの実行環境が上の条件3を満たすと確認できない（「Codexレビュー未実施」の記録がある、Codexの実行環境が書かれていない、modelやreasoning effortの記載がない、条件外のmodel等）: 手順2の「codex-consultationによるリストのレビュー」を行う

### 手順2: 初期セットアップ

#### targets.md がない場合、「調査対象リストを更新しろ」と指示された場合、または手順1で作り直しと判定した場合
...
Generated by: {Agent名 (model名)}, with {Codexの実行環境}
...
3. 調査結果に基づいて各観点をカスタマイズし、Writeツールで `.dev/codepatrol/checklist.md` に書き出す:
   - 冒頭に `Source commit: {git rev-parse --short HEAD の出力}` と `Generated by: {Agent名 (model名)}, with {Codexの実行環境}` を記録する。構成把握がどの時点・どのmodelによるものかを診断可能にする
...
「観点リスト（チェックリスト）を更新しろ」と指示された時、手順1で作り直しと判定した時、または既存の `.dev/codepatrol/checklist.md` を調査で使う前に、次を行う:
...
#### codex-consultationによるリストのレビュー

targets.mdまたはchecklist.mdを生成・更新したら、両方をファイルに書き出した上で、2つまとめてcodex-consultationでレビューする。
一方だけを更新した場合も、両方を渡す。
このレビューは省略も代替もできない。codex-consultationでのレビューを経ていないリストで手順3以降に進まない。

Skill toolで `codex-consultation` を呼び出す。Argsには以下を含める:

```
Args: よく相談して。このリポジトリの開発チームが、自分たちのコードの不具合を見つけて修正するためにセキュリティ調査をしている。その調査に使う調査対象リストと観点リストを作成した。以下の2つのファイルをレビューしてほしい。
...
Codexが回答を返さずに停止した場合は、手順5の「Codexが停止した場合」と同じ判断で作業を中断する。生成したファイルはそのまま残し、`Generated by` に「Codexレビュー未実施」と記録して、何のエラーで止まったかをユーザーに報告する。次の実行では手順1がこの記録を見つけ、このレビューから再開する。
...
`Generated by` には自分のAgent名とmodel名を記入し、codex-consultationが取得したCodexの実行環境を併記する。
...
### 手順5: codex-consultationによる批判的レビュー

手順4の調査結果を、codex-consultationを通じてCodexに批判させ、漏れ抜けを検出する。
...
```
Args: よく相談して。このリポジトリの開発チームが、自分たちのコードの不具合を見つけて修正するために{領域名}のセキュリティ調査をしている。
...
- 中断する時は、手順4の調査結果と、完了したグループのCodexの指摘・その検証結果を、リポジトリの作業ツリー外のファイルに書き出す。どのグループまで終わり、何のエラーで止まったかと、そのファイルのパスをユーザーに報告する
- 再開する時は、書き出した調査結果を起点に、手順5の残りのグループから続ける

Codexの指摘を受けたら:

- 指摘内容を自分でコードを読んで検証する
- 妥当な指摘は手順4の調査結果（＝レポートに書く発見）に反映する。Codexの指摘はbug寄りなので、checklistには取り込まない（取り込むとchecklistが発見リスト化し、次回以降の調査を特定箇所へ誘導してしまう。[checklist-vs-report.md](checklist-vs-report.md) 参照）
...
- **codex-consultation**: Codex CLIと相談するスキル。手順2のリストレビューと手順5の批判的レビューで使用する。必須で、他のスキルや `codex exec` の直接実行で代替しない

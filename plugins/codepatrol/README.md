...
リポジトリを領域ごとに巡回してセキュリティ調査するAgent Skillです。調査対象リストと観点チェックリストに基づいて未調査の領域を選んで調査し、外部Agentによる批判的レビューを経てレポートを出力します。
...
4. **外部Agentによる批判的レビュー**: 調査結果を外部Agentに批判させ、単独調査では見落とす攻撃ベクトルを検出します
...
- **`codex-consultation` スキル**: 手順5の批判的レビューで使用します

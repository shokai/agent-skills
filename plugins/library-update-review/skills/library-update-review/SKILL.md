...
### 3-2. package-lock.jsonの付随的変更の確認

package-lock.jsonの差分に含まれる**全パッケージの変更**を列挙し、以下に分類してください。

- **更新対象ライブラリ本体**: PRの目的と一致する変更
- **依存チェーン内の変更**: 更新対象ライブラリが依存するパッケージの変更（期待される副作用）
- **依存チェーン外の変更**: 更新対象ライブラリと無関係なパッケージの変更

依存チェーン外の変更が含まれている場合は、その変更を明記して警告してください。
それぞれの変更が意図したものかどうか、問題を引き起こすリスクがないかを調査してください。

> **注意**: `npm install` の実行タイミングやローカル環境の状態によって、更新対象と無関係なパッケージがpackage-lock.jsonに混入することがあります。これらが予期せぬ不具合を引き起こす可能性があります。

#### 調査手順

package-lock.jsonのdiffは数万行に及ぶことがあり、全体を目視確認するのは非現実的です。
以下のコマンドで**変更されたパッケージ名とバージョンを対で機械的に抽出**してから分類してください。

```bash
# PRのdiffから変更されたパッケージ名とバージョンを対で抽出する
# resolved URLにはパッケージ名とバージョンが含まれるため、version行より確実
gh pr diff <PR番号> --repo <owner>/<repo> 2>&1 \
  | grep -E '^[-+]\s+"resolved":' \
  | sort | uniq
```
...
- セクション3-2で特定した依存チェーン外の変更が、意図したものかどうか説明されているか

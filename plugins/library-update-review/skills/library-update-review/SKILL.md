...
### 3-2. ロックファイルの付随的変更の確認

ロックファイル（package-lock.json、Gemfile.lock等）の差分に含まれるパッケージ変更を、以下に分類してください。

- **更新対象ライブラリ本体**: PRの目的と一致する変更
- **依存チェーン内の変更**: 更新対象ライブラリが依存するパッケージの変更（期待される副作用）
- **依存チェーン外の変更**: 更新対象ライブラリと無関係なパッケージの変更

依存チェーン外の変更が含まれている場合は警告してください。
ロックファイルのdiffは巨大になることがあるため、変更されたパッケージ名とバージョンを機械的に抽出してから分類することを推奨します。

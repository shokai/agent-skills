...
実装者が自らPRをレビューするためのセルフレビューツールであり、人間のレビュアーに見せられる正気なpull requestに仕上げる事を目的とする。
...
- **チャット出力してpull requestにもコメント投稿する**（推奨）: 報告書を会話に出力し、同じ内容をPRにコメントとして投稿する
- **チャット出力する**: 報告書を会話に出力するのみ
...
ただし `# レビュー報告書` で始まるコメントは、このskillが過去に投稿した報告書なので対話コンテキストとして扱わない。
...
`# レビュー報告書` で始まるコメントは、実装者自身がこのskillで投稿した過去の報告書であり、実装の説明ではないので除外する。
...
手順0で「チャット出力してpull requestにもコメント投稿する」を選択している場合は、会話に出力した後、同じ内容をPRにコメントとして投稿する。

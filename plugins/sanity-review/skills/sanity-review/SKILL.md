...
手順3・手順4・手順5と手順6の6-6では外部Agentにセカンドオピニオンを求める（ただし手順4の「長期視点で命名・設計を考察する」サブセクションは対象外）。手順6のうち6-1〜6-5は疑わしい点がある場合に限り外部Agentへ相談する。以下のフォールバック順序に従い、**推測で判断せず実際に呼び出して試す**こと:
...
#### 6-6. 採用案の前提を1つずつ解除して再評価する

この手順が狙うのは「定石外発想」── 現状の発展型（既存案に機能や記述を足す方向）ではなく、前提という枷を外したときに立ち現れる解 ── である。

6-2・6-3が却下案・失敗試行（過去に捨てた選択肢）の判断の公平さを検証するのに対し、ここでは採用された設計そのものが置いている前提を組み替えられないかを検証する。

1. 採用案が依存している前提条件を、対話コンテキストとコードから列挙する
2. 各前提について「この前提が無ければ、よりシンプルな解になるか」を1つずつ検討する
3. シンプルになる場合は、採用案と次の観点で比較する: 実装量 / 既存仕様との互換性 / ユーザー・開発者への説明コスト / 運用・移行コスト / 将来の保守性
4. 結果を「今すぐ検討すべき」「将来の論点」「現状の採用案で妥当」に分類する

対話コンテキストやコードに根拠のない空想上の要件・前提は持ち込まず、レビュー対象の変更が依存している前提だけを扱う。

Agent自身の再評価に加えて、外部Agentにも前提解除を独立に行わせる。別の視点で前提を洗い出すことで、Agentが当然視して見落とした前提を拾える。外部Agentは放っておくと既存案のレビュー（現状の発展型）に流れるため、「レビュー」ではなく上記1〜4の手順そのもの（前提を自分で列挙し、1つずつ外してよりシンプルな解を探す）を実行させること。

「外部Agent相談の共通方針」に従い、以下のArgsで呼び出す:

```
Args: 軽く相談して。PR #{番号} の採用案が依存している前提を自分で列挙し、各前提を1つずつ外したらよりシンプルな解になるか検討してほしい（既存案のレビューではなく前提の組み替え）。対話コンテキストとコードに根拠のある前提だけを扱い、空想上の要件は持ち込まないこと。{採用案と対話コンテキストの要約}
```

外部Agentの別解を受け取ったら、自分の再評価と照合し、見落とした前提やより良い解がなかったか確認する。

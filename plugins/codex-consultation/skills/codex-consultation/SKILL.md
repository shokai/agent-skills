...
- 「codexがPR本文を取得できませんでした。今後PRレビューの相談時は、事前にClaudeがPR本文を取得してプロンプトに含めるようSKILL.mdを改良しませんか？」
- 「codexが特定のコマンドを実行できないようです。プロンプトに代替手段を指示する記述を追加しませんか？」
- 「codexがこのリポジトリのディレクトリ構成を把握できていないようです。プロンプトに主要なディレクトリ構成を含めるようにしませんか？」

### 進言のルール

- あくまで進言であり、SKILL.mdの変更はユーザーの承認を得てから行う
- 進言は具体的に行う。「何が失敗したか」「どう改良するか」「改良後のプロンプトやSKILL.mdの変更案」をセットで提示する

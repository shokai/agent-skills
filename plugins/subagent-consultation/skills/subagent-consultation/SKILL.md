...
2往復目を判断する前に、1往復目の出力に情報不足や不整合の兆候がないか確認する。問題を検知した場合は、まず相談元Agentが正しい情報を取得・補正してから2往復目の要否を判断する（詳細はセクション4「subagentの情報不足を検知した場合」を参照）。
...
### subagentの情報不足を検知した場合

subagentの出力を読む際、情報不足や不整合の兆候がないか注意深く確認する。
兆候の例: 事実と異なる記述、ファイルやコードの内容を誤認している、質問に対して的外れな回答をしている等。

問題を検知した場合は、以下の手順に従う:

1. まず、相談元Agentが正しい情報を自分で取得する。subagentが誤認した内容について、自分のツールで確認する
